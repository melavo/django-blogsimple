...
class BlogTestCase(TestCase):
    ...
    def test_bloginstall_redirect(self):
        ...
        response = self.c.get(reverse("blogsimple_index"))
    ...
    def test_blogindex_access(self):
        ...
        response = self.c.get(reverse('blogsimple_install'))
        self.assertEqual(response.status_code, 302)
...
class TestBlogEntry(TestCase):
    def setUp(self):
        ...
        self.blog.save()
        self.c = Client()
        self.user = User.objects.create_user(username='usertest', email='usertest@test.com', password='usertest')
...
class TestViews(TestCase):
    ...
    def test_entries_pagination(self):
        ...
        response = self.c.get(reverse('blogsimple_page', args=[1]))
        self.assertEqual(response.status_code, 200)
        response = self.c.get(reverse('blogsimple_page', args=[2]))
        self.assertEqual(response.status_code, 200)
        response = self.c.get(reverse('blogsimple_page', args=[3]))
    ...
    def test_tags_url(self):
        ...
        e2.tags.add("test1")
        response = self.c.get(reverse('blogsimple_tag_details', args=['test1']))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['entries']), 2)
        response = self.c.get(reverse('blogsimple_tag_details', args=['test2']))
        self.assertEqual(response.status_code, 200)
    ...
    def test_entry_existence(self):
        ...
        BlogEntry.objects.create(**{'title': 'test post',
            ...
            'publish_date': datetime.strptime("2017-01-22", "%Y-%m-%d"),
            'text_markup_type': "html",
            'created_by': self.user})
        response = self.c.get(reverse('blogsimple_details', args=['2017', '01', 'test-post']))
        self.assertEqual(response.status_code, 200)
    ...
    def test_add_comment(self):
        ...
        reponse = self.c.post(entry.get_absolute_url(), {'name': 'usertest', 'email': 'usertest@testtest.com',
    ...
    def test_author_entries_pagination(self):
        ...
        for each in range(num_entries):
            BlogEntry.objects.create(title="test", text='foo', created_by=self.user,
                    publish_date=datetime.today(), text_markup_type='plain')
        self.assertEqual(BlogEntry.objects.count(), 15)
        response = self.c.get(reverse('blogsimple_author_page', args=[self.user.username])+'?page=1')
        self.assertEqual(response.status_code, 200)
        response = self.c.get(reverse('blogsimple_author_page', args=[self.user.username])+'?page=2')
        self.assertEqual(response.status_code, 200)
        response = self.c.get(reverse('blogsimple_author_page', args=[self.user.username])+'?page=3')
        self.assertEqual(response.status_code, 404)
    ...
    def test_tagged_entries_pagination(self):
        ...
        self.assertEqual(BlogEntry.objects.count(), 15)
        response = self.c.get(reverse('blogsimple_tag_details_page', args=[tag.slug, 1]))
        self.assertEqual(response.status_code, 200)
        response = self.c.get(reverse('blogsimple_tag_details_page', args=[tag.slug, 2]))
        self.assertEqual(response.status_code, 200)
        response = self.c.get(reverse('blogsimple_tag_details_page', args=[tag.slug, 3]))
        self.assertEqual(response.status_code, 404)
...
class TestAdminActions(TestCase):
    ...
    def setUp(self):
        ...
        self.user.is_staff = True
        self.user.save()
        response = self.c.login(username='usertest', password='usertest')
    ...
    def test_check_adminpage(self):
        ...
        response = self.c.get(reverse('blogsimple_admin_dashboard'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed("admin/login.html")
        self.c.login(username='usertest', password='usertest')
        response = self.c.get(reverse('blogsimple_admin_dashboard'))
    ...
    def test_add_entry(self):
        ...
        response = self.c.post(reverse("blogsimple_admin_entry_new"), data)
        self.assertEqual(response.status_code, 200)
        ...
        data['text'] = 'test text'
        data['publish_date_0'] = '2017-01-22'
    ...
    def test_edit_entry(self):
        ...
        response = self.c.post("/blog/admin/entry/edit/%s/" % post.pk,
                ...
                'publish_date_0': '2017-01-22', 'publish_date_1': '17:17:55',
    ...
    def test_change_preferences(self):
        ...
        self.c.post(reverse('blogsimple_admin_edit_preferences'), {'title': 'new Blog', 'tag_line': 'my new blog',
                                                                           'entries_per_page': 10, 'recents': 5, 'recent_comments': 5})
    ...
    def test_admin_entrymanage(self):
        ...
        response = self.c.get(reverse('blogsimple_admin_entry_manage'))
        entries = response.context['entries']
        self.assertEqual(1, entries.count())

    def test_admin_author_entry_manage(self):
        """Check if we can manage entries by a specific author"""
        user = User.objects.create_superuser("test", "test@testtest.com", "test")
        create_test_blog_entry(user)
        response = self.c.get(reverse('blogsimple_admin_author_entry_manage', args=["test"]))
        entries = response.context['entries']
    ...
    def test_admin_commentmanage(self):
        ...
        response = self.c.get(reverse('blogsimple_admin_comments_manage'))
    ...
    def test_manage_entry_comments(self):
        ...
        create_test_comment(entry)
        response = self.c.get(reverse('blogsimple_admin_entry_comments_manage', args=[entry.pk]))
        comments = response.context['comments']
        self.assertEqual(1, comments.count())
...
class TestStaticFiles(TestCase):
    ...
    def test_images(self):
        abs_path = finders.find('blogsimple/images/agiliq_blog_logo.png')
        self.assertIsNotNone(abs_path)
        abs_path = finders.find('blogsimple/images/person_default.jpg')
        self.assertIsNotNone(abs_path)
        abs_path = finders.find('blogsimple/images/person_default.jpg')
        self.assertIsNone(abs_path)

    def test_css(self):
        abs_path = finders.find('blogsimple/css/as_blog_styles.css')
        self.assertIsNotNone(abs_path)
        abs_path = finders.find('blogsimple/css/prettify.css')
        self.assertIsNotNone(abs_path)
        abs_path = finders.find('blogsimple/css/prettify.css')
        self.assertIsNone(abs_path)
...
class TestFeedUrl(TestCase):
    ...
    def test_blog_feed(self):
        response = self.c.get(reverse("blogsimple_feed"))
        self.assertEqual(response.status_code, 200)
